...
def gen_addrSQL_seg(thisId):
    generIDinfo = randomIDtel.generatorIDC()

    nativePlace = generIDinfo['state'] + ''+ generIDinfo['city'] + ''+generIDinfo['district']
    idcode = generIDinfo['idc']

    SQLS = "UPDATE EMPLOYEE SET  NativePlace ='"+ nativePlace+"', IDCode='"+ idcode +"'  WHERE Id =" + str(thisId)
    return SQLS
